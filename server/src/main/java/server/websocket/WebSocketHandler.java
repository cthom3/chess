...
public class WebSocketHandler {
    ...
    public void onMessage(Session session, String message) throws IOException {
        if (message!=null) {
            ...
            switch (action.type()) {
                case PLAYER -> player(action.currentUser(), action.color(),session);
                case OBSERVER -> observer(action.currentUser(), session);
                case MOVE -> move(action.currentUser());
                case LEFT -> left(action.currentUser());
    ...
    private void player(String currentUser, String color, Session session) throws IOException {
        connections.add(currentUser, session);
        String message=String.format("%s joined the game as %s", currentUser, color);
    ...
    private void move(String currentUser) throws IOException {
    ...
    private void left(String currentUser) throws IOException {
    ...
    private void checkmate(String currentUser, Session session) throws IOException {
        String message=String.format("%s is in checkmate", currentUser);
        Notification notification = new Notification(Notification.Type.CHECKMATE, message);
        connections.broadcast(currentUser,notification);
    }
}
